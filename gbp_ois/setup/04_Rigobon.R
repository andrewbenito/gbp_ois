...
table.ss <- shocks_long |>
  filter(date == max(date)) |>
  dplyr::select(shock_origin, shock_value) |>
  mutate(shock_value = round(shock_value, digits = 2)) |> # Round to 2 decimal places
  arrange(desc(shock_value)) |>
  gt() |>
  cols_label(
