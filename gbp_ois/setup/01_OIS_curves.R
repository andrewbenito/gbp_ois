...
# Packages ----
lapply(c('here', 'tidyverse', 'readxl', 'lubridate', 'xts', 'showtext'),
...
p1 <- ggplot(fwcv, aes(x=date2, y = yield, group = date)) +
  ...
  labs(title = "GBP OIS Curves", 
       subtitle = "monthly averages of end-of-day daily data",
       x = "date", y = "rate %",
       caption = "Source: Bank of England data")
ggsave(here::here('plots', '1.GBP-OIS.png'), plot = p1, width = 9, height = 5, dpi = 90, units = "in", device='png')

# Recent data
comp.date <- last(fwcv$date) %m-% months(12)

p2 <- ggplot(subset(fwcv, date>=as.Date(comp.date)), aes(x=date2, y = yield, group = date)) +
  geom_line(aes(colour = as.factor(date))) + 
  theme(legend.position = "none") + 
  labs(title = "GBP OIS Curves: Past 12 months", 
       subtitle = "monthly averages of end-of-day daily data",
       x = "date", y = "rate %",
       caption = "Source: Bank of England data")
ggsave(here::here('plots', '2.GBP-OIS_12m.png'), plot = p2, width = 9, height = 5, dpi = 90, units = "in", device='png')

# save data
save(fwcv, file=here::here('data', 'data.Rda' ) )
