...
1. UK macro faces a higher incidence of negative supply shocks, which complicates the MPC's task of achieving the 2.0% inflation target. In this environment, government bonds provide a less effective hedge which would structurally weaken demand for bonds.  
2. Historical forecast error patterns suggest the MPC faces material upside inflation risks if it maintains current policy settings. The Bank's tendency to overestimate the pace of disinflation, combines with insufficient recognition of the UK's structural supply-side constraints. Inflation could persistently exceed the 2% target despite current monetary restriction.
3. Fiscal sustainability concerns amplify bond market volatility, particularly at the long-end and during data-sensitive periods. Disappointing economic data, coming from weak supply-side performance, simultaneously raise questions about fiscal capacity and inflation persistence. In these episodes, we should expect yield curve steepening. 
4. The authorities can mitigate yield curve steepening through active debt management, now a bigger feature of the UK policy, reducing the average maturity of new issuance. The second policy options available to contain steepening is that HMT prioritise forms of spending restraint and tax rises with lower implementation risk (eg re-profiling fixed investment plans). Front-loading offers similar benefits. 
...
6. Fiscal risk premium episodes tilt hawkish for the BoE. Our sense is that this view surprises most analysts. The BoE...
